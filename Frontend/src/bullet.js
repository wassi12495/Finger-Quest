...
class Bullet {
  ...
  static createBulletDiv() {
    ...
    bullet.innerHTML =
      '<img src="https://cdn.shopify.com/s/files/1/1061/1924/products/Thumbs_Down_Sign_Emoji_Icon_ios10_large.png?v=1513336434" style="width: 100%; height: 100%">';
  ...
  static checkCollision(bullet, bulletInterval) {
    ...
    let bulletRight =
      Bullet.positionToInteger(bullet.style.left) +
      Bullet.positionToInteger(BULLETWIDTH);
    ...
    if (
      (bulletLeft <= box1Left && bulletRight > box1Left && bottom >= 305) ||
      ...
      setTimeout(function() {
        ...
        setTimeout(function() {
          alert(
            `${Player.backpack[Player.backpack.length - 1].name}! ${
              Player.backpack[Player.backpack.length - 1].description
            }`
          );
          if (Player.ammo === 0) {
            Player.status = "loss";
      ...
      (bulletLeft <= box2Left && bulletRight > box2Left && bottom >= 305) ||
      ...
      (bulletLeft <= box3Left && bulletRight > box3Left && bottom >= 305) ||
